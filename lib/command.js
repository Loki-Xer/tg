...
async function command(message) {
    switch (message.command) {
        ...
        case "ping":
            ...
            const ping = await message.client.sendMessage(message.jid, "*Ping!*", { parse_mode: "Markdown" });
            const end = new Date().getTime();
            await message.client.editMessageText("*Pong!*\n" + (end - start) + " ms", { chat_id: message.jid, message_id: ping.message_id, parse_mode: "Markdown" });
            break;
        case "runtime":
